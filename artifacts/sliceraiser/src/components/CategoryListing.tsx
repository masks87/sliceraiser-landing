...
export type OpportunityCategory = "Property" | "Equity" | "Fixed Income";
...
export default function CategoryListing({ category, title, description, assetLabel = "Asset" }: CategoryListingProps) {
  const { data, isLoading, error } = useListOpportunities({ category });
  const initialAsset =
  ...
  const assetTypes = useMemo(
    () => ["All", ...Array.from(new Set(opportunities.map((o) => o.assetType)))],
    [opportunities],
  );
  const incomeTypes = useMemo(
    () => ["All", ...Array.from(new Set(opportunities.map((o) => o.incomeType)))],
    [opportunities],
  );
  ...
  return (
    <div className="bg-white py-12 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          <h1 className="text-[30px] font-bold text-[#4285f4] mb-3 leading-[36px]">{title}</h1>
          <p className="text-[#8e9196] text-base leading-[24px] max-w-2xl">{description}</p>
        </div>

        <div className="flex flex-wrap gap-3 mb-8">
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-semibold text-[#020817]">{assetLabel}</span>
        ...
        {!isLoading && !error && (
          <>
            ...
            {opportunities.length === 0 ? (
              <div className="bg-[#f7f9ff] border border-gray-100 rounded-xl p-10 text-center text-[#8e9196] text-sm">
                No opportunities available in this category yet. Check back soon.
              </div>
            ) : filtered.length === 0 ? (
              <div className="bg-[#f7f9ff] border border-gray-100 rounded-xl p-10 text-center text-[#8e9196] text-sm">
                No opportunities match the current filters. Try clearing one of the filters above.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filtered.map((o) => (
